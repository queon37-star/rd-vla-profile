...
The script validates planning artifacts and writes a fail-closed freeze. It does
not authorize GPU rollout; runtime source, checkpoint, environment, launcher,
and final analyzer are frozen separately.
"""
...
def cutoff_row(calibration: dict[str, Any], cutoff: float) -> dict[str, Any]:
    ...
    matches = [
        ...
        and abs(float(row.get("cutoff", -1.0)) - cutoff) <= 1e-12
    ]
    require(len(matches) == 1, f"expected one cutoff row for {cutoff}")
    return matches[0]


def validate_repeatability(payload: dict[str, Any]) -> None:
    require(
        payload.get("role") == "planning_repeatability_audit_not_performance_evidence",
        "wrong repeatability-audit role",
    )
    require(
        payload.get("behavioral_trace_repeatability_passed") is True,
        "repeatability audit did not pass",
    )
    require(payload.get("tasks") == [0, 5], "repeatability audit must cover tasks 0 and 5")
    comparisons = payload.get("comparisons")
    require(isinstance(comparisons, list) and len(comparisons) == 4, "expected four task-arm comparisons")
    require(
        {(int(row["task_id"]), str(row["arm"])) for row in comparisons}
        == {
            (0, "cold_initialized_adaptive"),
            (0, "midpoint_warm_start_adaptive"),
            (5, "cold_initialized_adaptive"),
            (5, "midpoint_warm_start_adaptive"),
        },
        "repeatability task-arm coverage mismatch",
    )
    require(all(int(row.get("pair_count", -1)) == 3 for row in comparisons), "expected three pairs per task-arm")
    unique_pairs = sum(int(row["pair_count"]) for row in comparisons) // 2
    require(unique_pairs == 6, "repeatability audit must represent six unique paired trials")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--cutoff-calibration",
        type=Path,
        default=Path("benchmark_results/warm_start_power/profile_cutoff_calibration_v1.json"),
    )
    parser.add_argument(
        "--state-accounting",
        type=Path,
    ...
    parser.add_argument(
        "--repeatability-audit",
        type=Path,
        default=Path("benchmark_results/warm_start_primary/preflight_repeatability_v1.json"),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    require(
        calibration.get("role") == "planning_profile_cutoff_calibration_not_final_authorization",
    ...
    require(abs(float(calibration.get("largest_qualified_cutoff", -1.0)) - CUTOFF) <= 1e-12, "largest qualified cutoff is not 0.045")
    ...
    selected = cutoff_row(calibration, CUTOFF)
    ...
    require(selected.get("all_zero_effect_power_scenarios_qualified") is True, "cutoff 0.045 failed power grid")
    ...
    counts = accounting.get("state_accounting")
    require(isinstance(counts, dict), "state accounting is incomplete")
    require(
        int(counts.get("maximum_warm_start_outcome_unseen_states_per_task", -1)) == PAIRS_PER_TASK,
        "state accounting does not expose 47 outcome-unseen states/task",
    )
    require(
        counts.get("preflight_states_allowed_in_primary_confirmatory_analysis") is False,
    ...
    require(int(preflight.get("total_pairs", -1)) == 30, "extended preflight must contain 30 pairs")
    require(preflight.get("warm_start_resets") == [], "warm-start resets were observed")
    validate_repeatability(repeatability)
    ...
    require(all(len(ids) == 3 for ids in excluded.values()), "expected three excluded states/task")
    require(all(len(ids) == PAIRS_PER_TASK for ids in included.values()), "expected 47 included states/task")
    require(
        all(not set(excluded[key]).intersection(included[key]) for key in included),
        "included/excluded state overlap",
    )
    ...
    payload = {
        ...
        "noninferiority": {
            ...
            "profile_p_value_cutoff": CUTOFF,
            "pass_rule": "one_sided_profile_p_value_strictly_less_than_0.045",
            "nominal_alpha_context": 0.05,
            "cutoff_interpretation": (
                "finite-sample simulation-calibrated cutoff for this fixed design; "
                "not a universal replacement alpha"
            ),
            "selected_cutoff_planning_results": {
                ...
                "minimum_zero_effect_power_wilson_95_low": selected["minimum_zero_effect_power_wilson_95_low"],
                "minimum_minus_one_point_power_sensitivity": selected["minimum_minus_one_point_power"],
            },
        ...
        "margin_rationale": {
            "substantive_threshold": (
                ...
                "rollouts is practically unacceptable"
            ),
            "continuity_with_prior_project_protocol": (
                "the earlier scalar-stopping preservation evaluation used the same "
                "5 percentage-point practical threshold"
            ),
            "selection_disclosure": (
                "5, 3, and 2 percentage-point candidates were assessed before final "
                "warm-start outcomes; 5 points was the maximum substantively accepted "
                "loss and the only candidate with adequate planned power at 47 pairs/task"
            ),
        },
        "state_allocation": {
            ...
            "excluded_initial_state_ids_by_task": excluded,
            "included_initial_state_ids_by_task": included,
            "acquisition_plan": {
                "runtime_phases": list(PRIMARY_PHASES),
                "executed_pairs_per_task": 50,
                "executed_episodes_both_arms": 1000,
                "primary_analysis_excludes_predeclared_first_three_calibration_states": True,
                "reason": "reuse validated 10/10/30 protocol without changing core state selection",
            },
        },
        "paired_seed_plan": {
            ...
            "seed_domain_note": (
                "the existing deterministic namespace is retained; new base seed and "
                "phase identities create the frozen final seed domain"
            ),
        },
        "secondary_efficiency_endpoints": {
            ...
            "trajectory_warning": (
                ...
                "length, and divergent closed-loop trajectories"
            ),
        },
        "artifact_provenance": {
        ...
        "forbidden_post_freeze_changes": [
            "noninferiority margin or cutoff",
            "primary method or state allocation",
            "midpoint definition or warm-start minimum iteration",
            "Action-MSE threshold 0.001 or max depth 32",
            "num_exec_actions 5",
            "task-specific rescue rules, staleness gates, latent/scalar stopping",
        ],
    ...
    print(
        "Frozen primary analysis: margin=0.05, pooled profile likelihood, "
        "cutoff=0.045, 47 pairs/task (470 total)"
    )
    print(
        "Acquisition: run 50 states/task through calibration+screening+final, "
        "exclude the frozen 3 preflight states/task from primary analysis"
    )
