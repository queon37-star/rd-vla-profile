...
from prismatic.models.action_heads import ActionHeadRecurrent
...
class GenerateConfig:
    ...
    use_minivla: bool = False                   # If True, uses MiniVLA model
    ...
    # fmt: on
    save_version: str = "rd-vla"                      # Version tag for saved results
...
def evaluate_policy(model, env, eval_sr_path, eval_result_path, num_procs, procs_id, eval_dir, ep_len, num_sequences, task_name='test', enrich_lang=False, debug=False):
    ...
    if enrich_lang:
        with open(os.path.join(os.path.dirname(__file__), 'enrich_lang_annotations.json'), 'r') as f:
...
def normalize_gripper_action(action: np.ndarray, binarize: bool = True) -> np.ndarray:
    ...
    if binarize:
        # Binarize to -1 or +1, treating 0 as +1
...
def main(cfg: GenerateConfig):
    ...
    eva = DualSystemCalvinEvaluation(model, proprio_projector, action_head, processor)
